        ...
        if (typeof req.pie !== 'undefined') {
           if (req.method === 'POST') {
               ...
               switch (req.pie.query.type) {
                ...
                case 'item':
                    if (req.pie.query.id) {
                        pie.db.channel(req.pie.user, domain).getItem('root', req.pie.channel, req.pie.query.id, function (err, reply) {
                            if (err) {
                                res.send(400, err);
                            } else {
                                res.send(200, reply);
               ...
               switch (req.pie.query.type) {
                   case 'item':
                        pie.db.channel(req.pie.user, domain).delItem('root', req.pie.channel, req.pie.query.id, function (err, reply) {
                            if (err) {
                                res.send(400, err);
                            } else {
                                res.send(200, reply);
