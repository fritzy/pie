    ...
    this.exp.use('/pie', (req, res, next) => {
        ...
        if (typeof req.session.user === 'undefined') {
            if (req.headers.authorization) {
                ...
                this.pie.db.auth(req.host).validateToken(token, (err, user, domain) => {
                    if (err) {
                ...
                req.on('end', () => {
                    ...
                    if (query.hasOwnProperty('getToken')) {
                        pie.db.auth(req.host).getToken(jdata.user, jdata.password, (err, token) => {
                            if (err || !token) {
                                res.send(401, "Invalid credentials");
                            } else {
                                res.set('Content-Type', 'application/json');
                                res.send(200, {user: jdata.user, token});
                            }
                        });
                    } else if (query.hasOwnProperty('simpleAuth')) {
                        pie.db.auth(req.host).verifyPassword(query.user, query.password, (err, passed) => {
                            if (err || !passed) {
                                res.send(401, "Invalid credentials");
                            } else {
           ...
           req.on('end', () => {
               ...
               if (opts.query.hasOwnProperty('rpc')) {
                   ...
                   if (opts.query.rpc.indexOf('Subscription') != -1) {
                       ...
                       this.pie.subscription.handleHTTP(opts, (err, result) => {
                           res.send(200, result);
                       });
                   }
               } else {
                   ...
                   try {
                       pie.db.channel(req.pie.user, domain)[methodf](opts, (err, reply, code, ctype) => {
                           if (ctype) {
