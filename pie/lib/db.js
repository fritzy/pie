...
(function () {
    ...
    this.getToken = function (user, password, cb, skippass) {
        ...
        this.verifyPassword(user, password, function (err, passed) {
            if (passed || skippass) {
                this.db.atomic(function (done) {
                    this.get('tokens::' + '_' + authdomain, function (err, tokens) {
                        if (!tokens) {
                            tokens = {};
                        }
                        var token = 'auth:' + uuid();
                        ...
                        console.log(tokens);
                        this.put('tokens::' + '_' + authdomain, tokens, function (err) {
                            done();
                            cb(false, token);
                        });
                    }.bind(this));
    ...
    this.validateToken = function (token, cb) {
        this.db.get('tokens::_' + this.domain, function (err, tokens) {
            var user;
            if (err || !tokens) {
                cb("Not authorized.");
            } else {
